...
class neptune_meta_box {
	...
	function __construct($meta_box) {
		...
		$this->check_field_slider();
		$this->check_field_single_image();
	...
	function js_css() {
		...
		$base_url = get_template_directory_uri(). '/functions/meta/';	// or inside a theme (note that this statement WON'T do anything if the above statement was successful)

		wp_enqueue_style('neptune-meta-box', $base_url . 'meta-box.css');
		wp_enqueue_script('neptune-meta-box', $base_url . 'meta-box.js', array('jquery'), null, true);
	}
	...
	function check_field_upload() {
		...
		add_filter('media_upload_album', array(&$this, 'insert_images'));			// process adding multiple images to image meta field
	...
	function insert_images() {
		if (!isset($_POST['neptune-insert']) || empty($_POST['attachments'])) return;
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image', 'neptune') . "' class='neptune-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete', 'neptune') . "</a>";
	...
	function check_field_date() {
		if ($this->has_field('date') && $this->is_edit_page()) {
			...
			wp_enqueue_style('neptune-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('neptune-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && $this->is_edit_page()) {
			...
			wp_enqueue_style('neptune-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('neptune-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('neptune-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('neptune-jquery-ui'));
		}
	}
	
	// Check field time
	function check_field_single_image() {
		// make upload feature works even when custom post type doesn't support 'editor'
		wp_enqueue_script('media-upload');
		add_thickbox();
	}
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'neptune_meta_box_nonce');
	...
	function show_field_begin($field, $meta) {
		echo "<th class='neptune-label-td'><label class='neptune-label' for='{$field['id']}'>{$field['name']}</label></th><td class='neptune-field' style='position: relative;'>";
	}
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='neptune-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='neptune-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='5'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select class='neptune-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . ">";
	...
	function show_field_radio($field, $meta) {
		$this->show_field_begin($field, $meta);
		foreach ($field['options'] as $key => $value) {
			echo "<input type='radio' class='neptune-radio' name='{$field['id']}' value='$key'" . checked($meta, $key, false) . " /> $value ";
		}
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='checkbox' class='neptune-checkbox' name='{$field['id']}' id='{$field['id']}'" . checked(!empty($meta), true, false) . " /> {$field['desc']}</td>";
	}

	function show_field_wysiwyg($field, $meta) {
		$this->show_field_begin($field, $meta);
		//wp_tiny_mce(true, array('editor_selector' => 'neptune-rich-editor', 'remove_linebreaks' => false) );
		echo "<textarea class='neptune-wysiwyg neptune-rich-editor large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			$nonce = wp_create_nonce('rw_ajax_delete');
			echo '<div style="margin-bottom: 10px"><strong>' . __('Uploaded files', 'neptune') . '</strong></div>';
			echo '<ol class="neptune-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='neptune-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete', 'neptune') . "</a>)</li>";
			}
			echo '</ol>';
		}

		// show form upload
		echo "<div style='clear: both'><strong>" . __('Upload new files', 'neptune') . "</strong></div>
			<div class='new-files'>
				<div class='file-input'><input type='file' name='{$field['id']}[]' /></div>
				<a class='neptune-add-file' href='#'>" . __('Add more file', 'neptune') . "</a>
			</div>
		</td>";
	}
	...
	function show_field_image($field, $meta) {
		...
		echo "<input type='hidden' class='neptune-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='neptune-images neptune-upload' id='neptune-images-{$field['id']}'>";
		...
		foreach ($images as $image) {
			...
			echo "<li id='item_$image'>
					<img src='$src' />
					<a title='" . __('Delete this image', 'neptune') . "' class='neptune-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete', 'neptune') . "</a>
					<input type='hidden' name='{$field['id']}[]' value='$image' />
				</li>";
		}
		echo '</ul>';

		echo "<a href='#' class='neptune-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images', 'neptune') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='neptune-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='neptune-color-select' rel='{$field['id']}'>" . __('Select a color', 'neptune') . "</a>
			  <a href='#' class='neptune-color-select' style='display: none;'>" . __('Hide Picker', 'neptune') . "</a>
			  <div style='display:none; position: absolute; background: #f0f0f0; border: 1px solid #ccc; z-index: 100;' class='neptune-color-picker' rel='{$field['id']}'></div>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='neptune-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='neptune-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='neptune-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_single_image($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='neptune-single-image' name='{$field['id']}' id='{$field['id']}' value='$meta' size='18' />";
		echo "<input type='button' class='neptune-upload-single-image button-secondary' value='Choose Image'/><br />";
		$this->show_field_end($field, $meta);
	}
	
	function show_field_slider($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<div class='neptune-slider' rel='{$field['id']}'></div>";		
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'neptune_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('neptune_meta_box_Validate') && method_exists('neptune_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('neptune_meta_box_Validate', $field['validate_func']), $new);
			}
